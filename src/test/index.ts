...
describe(`Test 'OUTPOSTS_API' service.`, () => {
  let outpostsVision
  before(() => {
    outpostsVision = Exovision('OUTPOSTS_API')
  })
  it(`GET outposts list`, async () => {
    const outposts = await outpostsVision
      .get('/api/v1/outposts')
    expect(outposts).to.deep.equal({ data: { outposts: [], totalCount: 0 } })
  })

  it(`POST to outposts (create)`, async () => {
    const outpost = await outpostsVision
      .post('/api/v1/outposts', { name: 'kapa' })
  ...
  it(`GET single outpost`, async () => {
    const outpost = await outpostsVision
      .get('/api/v1/outposts/5b22ef3881af6d53c4adcec4')
    expect(outpost).to.deep.equal({
      _id: '5b22ef3881af6d53c4adcec4',
      name: 'kapa',
    })
  ...
  it(`PUT single outpost (modify)`, async () => {
    const outpost = await outpostsVision
      .put('/api/v1/outposts/5b22ef3881af6d53c4adcec4', { name: 'kapa2' })
    expect(outpost).to.deep.equal({
      _id: '5b22ef3881af6d53c4adcec4',
      name: 'kapa2',
    })
  ...
  it(`DELETE single outpost`, async () => {
    const outpost = await outpostsVision
      .del('/api/v1/outposts/5b22ef3881af6d53c4adcec4')
    expect(outpost).to.deep.equal({})
  })

  it(`GET no answer on /outposts`, async () => {
    try {
      await outpostsVision.put('/api/v1/outposts/noanswer')
    } catch (err) {
      expect(err.code).to.equals('REMOTE_UNREACHABLE')
  ...
  it(`GET no resource on /outposts`, async () => {
    try {
      ...
      expect(err.code).to.equals('REMOTE_API_ERROR')
